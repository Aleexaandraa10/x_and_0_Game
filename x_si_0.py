...
class InfoJoc:
    ...
    """
    NR_COLOANE=3
    JMIN=None #simbol jucator x
    JMAX=None #simbol jucator 0
    GOL='#'
        ...
        for i, linie in enumerate(self.matr):
            for j,elem in enumerate(linie):
                if elem == InfoJoc.GOL:
                    ...
                    lMutari.append(InfoJoc(tablaNoua))
        return lMutari
...
class Stare:
    ...
    De asemenea cere ca in clasa InfoJoc sa fie definita si o metoda numita mutari() care ofera lista cu
...
# scop: gaseste mutarea optima pentru jucatorul curent(j_curent)
def min_max(stare):
    if stare.tabla_joc.final() or stare.adancime == 0:
        stare.estimare = stare.tabla_joc.estimeaza_scor(stare.adancime)
        return stare
    mutariCuEstimare= [min_max(mutare) for mutare in stare.mutari()]
    # daca jucatorul curent e AI
    if stare.j_curent == InfoJoc.JMAX:
        stare.stare_aleasa = max(mutariCuEstimare, key = lambda x: x.estimare)
    else: # adversarul incearca sa minimizeze sansele AI de a castiga
        stare.stare_aleasa = min(mutariCuEstimare, key = lambda x: x.estimare)
    stare.estimare = stare.stare_aleasa.estimare
    return stare


# acelasi lucru ca MinMax, dar e mai eficient
# taie(pruneaza) mutarile care nu pot fi mai bune decat ce avem deja
# alpha: cea mai buna valoare (cea mai mare) pe care AI(MAX) a gasit-o pe parcursul cautarii
# beta: cea mai buna valoare (cea mai mica) pe care adversarul (MIN) a gasit-o pe parcursul cautarii
        ...
        for mutare in stare.mutari():
            ...
            if stare.estimare < mutareCuEstimare.estimare:
                ...
                if alpha < mutareCuEstimare.estimare:
                    ...
                    #alta mutare este deja mai buna, nu mai are rost sa continuam pe ramura asta
                    if beta <= alpha:
                        break
    else:
        stare.estimare = float('inf')
